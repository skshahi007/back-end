...
export const updateOrCreateGoogleOauth = async ({ oAuthUserInfo }) => {
    ...
    const existingUser = await db.collection('admin-auth').findOne({ email });

    if (existingUser) {
        const result = await db.collection('admin-auth').findOneAndUpdate(
            { email },
        ...
        const result = await db.collection('admin-auth').insertOne(data)
